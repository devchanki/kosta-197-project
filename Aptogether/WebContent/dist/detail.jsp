...
<div class="card-body">
     ...
     <div>
         <!-- •2020-04-06 14:11:45 -->
         <fmt:parseDate var="date" value="${product.regdate }" pattern="yyyy-MM-dd HH:mm:ss"/>
      ...
      <%-- <a href="/Aptogether/download.jsp?filename=${product.fname}">${product.fname}</a> --%>
       	<c:if test="${product.fname != null }">
						<c:set var="head" value="${fn:substring(product.fname, 
												0, fn:length(product.fname)-4) }"></c:set>
						<c:set var="pattern" value="${fn:substring(product.fname, 
...
<%--    <c:forEach var="reply" items="${replys}">
   ...
   <a href="updateform.do?seq=${ product.seq }">수정</a>
   <a href="deleteaction.do?seq=${ product.seq }">삭제</a>
     </div>
            </div>
                    </div>
                </main>
